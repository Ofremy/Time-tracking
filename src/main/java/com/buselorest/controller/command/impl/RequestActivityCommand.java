...
public class RequestActivityCommand implements Command {
    ...
    private final static Logger logger = Logger.getLogger(RequestActivityCommand.class);

    public RequestActivityCommand(ServiceFactory serviceFactory, DBConnectionPool pool) {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection connection = pool.getConnection()) {
            ...
            for (Request request1 : (List<Request>) request.getSession().getAttribute("requestList")) {
                if (request1.getUser().getId() == user.getId() && request1.getActivityName().equals(name)) {
                    return PageConstants.USER_ACTIVITY;
                }
            }
            Request req = requestService.create(new Request(user,
                    name,
                    request.getParameter("description"),
                    Request.Status.ADD), connection);
            logger.info("User with id " + user.getId() + " request for create new Activity with name" + req.getActivityName());
